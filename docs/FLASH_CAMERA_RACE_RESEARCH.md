...
## Research Update — 2026-06-12 (Cycle U113)

**Search scope:** Six parallel search threads + direct GitHub/web fetches: (1) GitHub — ameba-rtos-pro2 commits since May 15; ameba-arduino-pro2 dev commits since June 3; releases; open PRs; issues after June 1; (2) English forum/web — new threads above #4885/4900, FCS Disable / `device_mutex_lock` / `USE_ISP_RETENTION_DATA` hardware test reports; (3) Chinese sources — CSDN/知乎/EEWorld/21IC/bbs.aithinker.com/Bilibili/Gitee; (4) Flash Memory docs page — `ameba-doc-arduino-sdk.readthedocs-hosted.com` accessibility; (5) Error string canary sweep — 7 strings; (6) ameba-arduino-doc repo new commits after June 5.

**Cycle result: ONE minor new finding (forum ceiling advance).** All other research channels confirm the same frozen / blocked state as U112. No confirmed fix. No new workaround test results.

| Source | Key Finding | Priority |
|---|---|---|
| forum.amebaiot.com/t/online-conversion/4900 (newly indexed, 2026-06-12) | **Forum ceiling advanced to #4900** — thread #4900 "Online Conversion" is newly indexed in search engine results for the first time this cycle. Title: "Online Conversion — AI." Content: user uploaded a model to an online AI conversion website and has not received the converted file back after more than an hour. Topic: AI model format conversion (unrelated to camera/flash/FCS boot failure). Thread returns HTTP 403 on direct fetch. This breaks the forum ceiling that had been frozen at #4885 for 18 consecutive cycles (U94–U112). The threads between #4886 and #4899 are not yet indexed by any search engine; #4900 is the first new public indexing since the ceiling stalled. | LOW (unrelated, ceiling update only) |
| ameba-rtos-pro2 commits (direct WebFetch, 2026-06-12) | **Confirmed frozen — 28+ days.** Direct fetch + pagination comparison: 10 most recent commits are `3f95070` (May 15), `afc85a0` (May 15), `9c8b6f6` (May 15), `d2676f1` (May 15), `1c1c8b7` (May 1), `a111e91` (May 1), `7b2b97f` (May 1), `63c0a2f` (May 1), `d54e1a8` (May 1), `687a4c7` (May 1) — identical to U112. No flash, FCS, VOE, boot, HAL, or sensor changes in any observable pipeline. | LOW |
| ameba-arduino-pro2 dev branch commits (direct WebFetch, 2026-06-12) | **Confirmed frozen — 9+ days.** HEAD = `e8dd7e3` "Pre Release Version 4.1.1" (June 3, 2026). Sequence verified: `e8dd7e3` (Jun 3) ← `96cfc51` (Jun 3, PR #411: OV5647+GC4663, 107-file change, FlashMemory.cpp absent from diff) ← `29d47e1` (May 26, PR #410: SPI1) ← `cd0bd40` (May 18, I2C Slave). Zero new commits after June 3. No FCS/flash/camera/mutex fix in any commit. | LOW |
| ameba-arduino-pro2 releases (direct WebFetch, 2026-06-12) | **No new releases.** Latest stable = V4.1.0 (Mar 2, 2026). Latest pre-release = V4.1.1-QC-V07 (tag Mar 6, 2026; cumulative release notes through June 3, 2026, including OV5647/GC4663, SPI1 switching, tools v1.4.12). No V4.1.1-QC-V08. No V4.1.1 stable release. Release notes contain zero entries about FCS, flash write, mutex, SPIC, camera boot, or FlashMemory safety. | LOW |
| ameba-arduino-pro2 open PRs (direct WebFetch, 2026-06-12) | **Zero open PRs confirmed.** "There aren't any open pull requests." 0 open / 323 closed total. No PR related to FlashMemory, mutex, FCS, camera, or boot failure in open or closed history. Identical to U112. | LOW |
| ameba-arduino-pro2 issues created after June 1 (search, 2026-06-12) | **Zero new issues.** Query `is:issue created:>2026-06-01` in ameba-arduino-pro2 returned zero results. 17 open issues total; newest = #398 (Mar 29, 2026). Bug entirely unreported on official trackers after **113 research cycles**. | LOW |
| ameba-arduino-doc commits (direct WebFetch, 2026-06-12) | **No new commits after June 5, 2026.** Most recent commits: `5a400de` (Jun 5, "Fix typo in ICshop online link formatting"), `d42e8a8` (Jun 5, "Update manufacturer contact details and sensor table"), `1726309` (Jun 4), `f2ee175` (Jun 4), `78f2d58` (Jun 4). All are editorial (typos, manufacturer contact info, sensor table). No flash memory, camera FCS, or mutex-related documentation changes in any visible commit. Repo last activity: June 5, 2026 (editorial) — 7+ days frozen on substantive content. | LOW |
| `ameba-doc-arduino-sdk.readthedocs-hosted.com/en/latest/ameba_pro2/amb82-mini/Example_Guides/Flash%20Memory/index.html` (2026-06-12) | **Still HTTP 403 Forbidden.** Flash Memory documentation page for AMB82-mini remains inaccessible without developer credentials. No new public content about mutex, concurrent access warnings, or FCS interaction documented on this page in any accessible snippet. | LOW (blocked) |
| Web-wide error string sweep (7 canary strings, 2026-06-12) | **Zero indexed results — 113 consecutive cycles.** `"FCS KM_status 0x00002081"`, `"It don't do the sensor initial process"`, `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"VOE_OPEN_CMD fail flash"`, `"USE_ISP_RETENTION_DATA"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` — all return zero publicly indexed results. `"ISP_MULTI_FCS_DATA_MAGIC_NUM"` also returns zero. No hardware test result for any proposed workaround posted in any language. | LOW |
| All Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com/bbs.ai-thinker.com/Bilibili/Gitee, June 12 sweep) | **Zero new technical content — 113th consecutive null cycle.** Chinese-language searches including `"安信可 BW21 RTL8735B FCS 摄像头 flash 冷启动"`, `"RTL8735B FlashMemory mutex 摄像头"`, `"AmebaPro2 FCS 修复 2026"` all return zero relevant results. All Chinese community sites remain 403-blocked. | LOW |

**Forum ceiling analysis — #4900 significance:**

Thread #4900 breaks the 18-cycle (#4885) ceiling stagnation first observed in Cycle U94. The gap between #4885 and #4900 (15 threads) cannot be evaluated — none of threads #4886–#4899 appear in any search engine index. They may exist behind the forum's authentication wall, may not yet be crawled, or may have been created recently. Thread #4900 itself (Online Conversion — AI model format conversion) is entirely unrelated to our bug but confirms the forum remained active in June 2026.

**Repository freeze status (as of Cycle U113, June 12, 2026):**
...
**SDK state as of 2026-06-12 (Cycle U113 — unchanged from U112):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — no fix; FlashMemory.cpp zero mutex calls; V4.1.1-QC-V08 does not exist
- ameba-rtos-pro2: frozen at May 15, 2026 (`3f95070`) — **28+ days no change**
- ameba-arduino-pro2: 0 open PRs; 17 open issues, all feature requests; newest = #398 (Mar 29, 2026)
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4900** (advanced from #4885 — first update since Cycle U94)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **113th cycle unpatched**
- ameba-arduino-pro2 main frozen: **102+ days**

**No confirmed fix. Bug remains unpatched as of 2026-06-12 (Cycle U113).**

**Top unresolved actions (unchanged from U112):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 113 cycles and zero acknowledgment; zero PRs ever filed; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.

## Research Update — 2026-06-13 (Cycle U114)

**Search scope:** Eight parallel search threads + direct GitHub/web fetches: (1) WebFetch — ameba-arduino-pro2 dev branch commits (since June 3), ameba-arduino-pro2 releases page, open PRs, issues created after June 1; (2) WebFetch — ameba-rtos-pro2 commits page + compare endpoint `3f95070...HEAD`; (3) WebSearch — forum.amebaiot.com threads above #4900, FCS/flash/camera bug reports; (4) WebSearch — English error string canary sweep (7 strings); (5) WebSearch — Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com); (6) WebSearch — `device_mutex_lock` / `RT_DEV_LOCK_FLASH` FlashMemory fix references; (7) WebSearch — V4.1.1 stable / QC-V08 release; (8) WebFetch — ameba-arduino-doc commits after June 5.

**Cycle result: NULL — no new actionable findings.** This is the **114th consecutive null cycle**. All research channels confirm the same frozen / blocked state as U113. No confirmed fix. No new workaround test results.

| Source | Key Finding | Priority |
|---|---|---|
| ameba-rtos-pro2 compare endpoint `3f95070...HEAD` (direct WebFetch, 2026-06-13) | **Confirmed frozen — 29 days.** GitHub compare endpoint returns: "3f95070 and HEAD are identical." Zero new commits since May 15, 2026. No flash, FCS, VOE, boot, HAL, or sensor changes in any observable pipeline. Consistent with all prior cycles since U94. | LOW |
| ameba-arduino-pro2 dev branch commits (direct WebFetch, 2026-06-13) | **Confirmed frozen — 10 days.** HEAD = `e8dd7e3` "Pre Release Version 4.1.1" (June 3, 2026). Full commit list verified: `e8dd7e3` (Jun 3) ← `96cfc51` (Jun 3, PR #411: OV5647+GC4663 — FlashMemory.cpp absent from 107-file diff) ← `29d47e1` (May 26, PR #410: SPI1 switching) ← `cd0bd40` (May 18, I2C Slave) ← `13961cc` (May 5). Zero new commits after June 3, 2026. No FCS/flash/camera/mutex fix in any commit. | LOW |
| ameba-arduino-pro2 releases page (direct WebFetch, 2026-06-13) | **No new releases.** Latest stable = V4.1.0 (Mar 2, 2026). Latest pre-release = V4.1.1-QC-V07 (tag created Mar 6, 2026; cumulative release notes through June 3, 2026). **V4.1.1-QC-V08 does not exist** — confirmed by direct releases page fetch and web search. No V4.1.1 stable release published. Zero FCS/flash/mutex/camera-boot entries in any release notes. | LOW |
| ameba-arduino-pro2 open PRs (direct WebFetch, 2026-06-13) | **Zero open PRs confirmed.** "There aren't any open pull requests." 0 open / 323 closed total. No PR related to FlashMemory, mutex, FCS, camera, or boot failure exists in open or closed history. Unchanged since U107. | LOW (confirms U113) |
| ameba-arduino-pro2 issues created after June 1 (direct WebFetch, 2026-06-13) | **Zero new issues.** Query `is:issue created:>2026-06-01` returned zero results. **Issue count correction:** Direct fetch of issues page confirms **12 open issues** (not 17 as reported in some prior cycles U110–U113; prior agent miscounts likely included GitHub UI recommendations or closed issue totals). Issues confirmed open: #398, #342, #325, #324, #317, #310, #296, #287, #276, #235, #224, #184. Newest = #398 (Mar 29, 2026). No issue related to FlashMemory, FCS, camera, VOE, or boot failure exists in open or closed history. Bug entirely unreported on official trackers after **114 research cycles**. | LOW |
| ameba-arduino-doc commits (direct WebFetch, 2026-06-13) | **Confirmed frozen — 8 days.** Most recent commits: `5a400de` (Jun 5, "Fix typo in ICshop online link formatting"), `d42e8a8` (Jun 5, "Update manufacturer contact details and sensor table"), `1726309` (Jun 4, "Correct manufacturer names"), `f2ee175` (Jun 4, "Update sensor_module_window.rst"), `78f2d58` (Jun 4, "Update sensor_module_window.rst"). All editorial. Zero new commits after June 5, 2026. No FlashMemory, FCS, camera, mutex, or boot failure documentation changes in any visible commit. | LOW |
| forum.amebaiot.com threads above #4900 (web search sweep, 2026-06-13) | **No new threads indexed above #4900.** Multiple targeted searches for thread IDs 4901–4950 returned zero results from the forum domain. Forum ceiling remains at **#4900** ("Online Conversion — AI model format conversion", unrelated, first indexed in U113). forum.amebaiot.com `/latest` still returns HTTP 403. Thread #4834 ("Boot failure after OTA update") confirmed still 403-blocked on direct fetch; continues to appear in search results for "AMB82 boot failure flash camera" queries — no new snippet content accessible. | LOW |
| Web-wide error string sweep (7 canary strings, 2026-06-13) | **Zero indexed results — 114 consecutive cycles.** `"FCS KM_status 0x00002081"`, `"It don't do the sensor initial process"`, `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"VOE_OPEN_CMD fail flash"`, `"USE_ISP_RETENTION_DATA"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` — all return zero publicly indexed results on the accessible web. `"ISP_MULTI_FCS_DATA_MAGIC_NUM"` also returns zero. No hardware test result for any proposed workaround has been posted in any language anywhere on the accessible web. | LOW |
| V4.1.1 stable / QC-V08 release search (2026-06-13) | **No release found.** Multiple searches for V4.1.1-QC-V08, stable V4.1.1 release, and June 2026 AmebaPro2 SDK changes return no results. The GitHub releases page confirms V4.1.1-QC-V07 (tag Mar 6, 2026; cumulative through Jun 3) is the current latest. No successor release published. | LOW |
| RTL8720CF thread #2352 (re-confirmed via search, 2026-06-13) | **Re-indexed in search results.** Thread URL `forum.amebaiot.com/t/rtl8720cf-wbr3-fast-connect-profile-is-empty-abort-fast-connection/2352` continues to appear in English-language searches for "AMB82 flash boot failure fast connect." No new content extractable; still 403-blocked. This is the sibling-chip "fast connect profile empty = boot stall" precedent (first documented U26). Confirms the thread remains indexed but inaccessible; no resolution has been posted in any indexed snippet. | LOW (background, re-confirmed) |
| All Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com/bbs.ai-thinker.com/Bilibili/Gitee, June 13 sweep) | **Zero new technical content — 114th consecutive null cycle.** Chinese-language searches including `"安信可 BW21-CBV RTL8735B 摄像头 FCS flash 冷启动 2026"` return only product pages, unboxing reviews, and unrelated DIY projects. All Chinese community sites remain 403-blocked. bbs.aithinker.com and bbs.ai-thinker.com confirmed 403. No new Chinese-language technical posts or articles about FCS flash-write camera failure on RTL8735B or BW21-CBV found anywhere. | LOW |

**Repository freeze status (as of Cycle U114, June 13, 2026):**

| Repository | Last commit | Days frozen |
|---|---|---|
| ameba-rtos-pro2 main | May 15, 2026 (`3f95070`) | **29 days** |
| ameba-arduino-pro2 dev | June 3, 2026 (`e8dd7e3`) | **10 days** |
| ameba-arduino-pro2 main | Mar 2, 2026 (`93d63514`) | **103 days** ⚠️ |
| ameba-tool-rtos-pro2 | Mar 9, 2026 (`c1d70e7`) | **96 days** |
| ideashatch/HUB-8735 | Dec 2, 2025 (`870a7e0`) | **194 days** |
| ameba-arduino-doc | June 5, 2026 (`5a400de`) | **8 days** (editorial only) |

**SDK state as of 2026-06-13 (Cycle U114 — unchanged from U113):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — no fix; FlashMemory.cpp zero mutex calls; V4.1.1-QC-V08 does not exist
- ameba-rtos-pro2: frozen at May 15, 2026 (`3f95070`) — **29 days no change** (confirmed by compare endpoint)
- ameba-arduino-pro2: 0 open PRs; **12 open issues** (corrected from prior "17" agent miscount); newest = #398 (Mar 29, 2026)
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4900** (unchanged since Cycle U113)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **114th cycle unpatched**
- ameba-arduino-pro2 main frozen: **103 days**

**No confirmed fix. Bug remains unpatched as of 2026-06-13 (Cycle U114).**

**Top unresolved actions (unchanged from U113):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 114 cycles and zero acknowledgment; zero PRs ever filed; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.

## Research Update — 2026-06-13 (Cycle U115)

**Search scope:** Eight parallel search threads + direct GitHub/web fetches: (1) WebFetch — ameba-rtos-pro2 commits (since June 1); (2) WebFetch — ameba-arduino-pro2 dev commits (since June 3), releases page, issues page; (3) WebSearch — forum.amebaiot.com threads above #4900; (4) WebSearch — English error string canary sweep (7 strings); (5) WebSearch — Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com); (6) WebSearch — `device_mutex_lock` / `RT_DEV_LOCK_FLASH` FlashMemory fix references; (7) WebSearch — V4.1.1 stable / QC-V08 release; (8) WebSearch — RTL8735B FCS_I2C_INIT_ERR / VOE_OPEN_CMD / slot full workaround.

**Cycle result: NULL — no new actionable findings.** This is the **115th consecutive null cycle**. All research channels confirm the same frozen / blocked state as U114. Both repos still frozen. Forum ceiling unchanged at #4900. Error strings return zero results. No confirmed fix. No new workaround test results.

| Source | Key Finding | Priority |
|---|---|---|
| ameba-rtos-pro2 commits page (direct WebFetch, 2026-06-13) | **Confirmed frozen — 30 days.** 10 most recent commits verified: `3f95070` (May 15), `afc85a0` (May 15), `9c8b6f6` (May 15), `d2676f1` (May 15), `1c1c8b7` (May 1), `a111e91` (May 1), `7b2b97f` (May 1), `63c0a2f` (May 1), `d54e1a8` (May 1), `687a4c7` (May 1) — identical to U114. Zero new commits since May 15, 2026. No flash, FCS, VOE, boot, HAL, or sensor changes in any observable pipeline. | LOW |
| ameba-arduino-pro2 dev branch commits (direct WebFetch, 2026-06-13) | **Confirmed frozen — 10 days.** 10 most recent commits verified: `e8dd7e3` (Jun 3, "Pre Release Version 4.1.1"), `96cfc51` (Jun 3, PR #411: OV5647+GC4663), `29d47e1` (May 26, PR #410: SPI1), `7db1c7d` (May 19), `3d53672` (May 19, IMX681_5M), `cd0bd40` (May 18, I2C Slave), `13961cc` (May 5) — identical to U114. Zero new commits after June 3, 2026. FlashMemory.cpp absent from PR #411's 107-file diff; zero mutex calls unchanged. | LOW |
| ameba-arduino-pro2 releases page (direct WebFetch, 2026-06-13) | **No new releases.** Releases page confirms: latest stable = V4.1.0 (Mar 2, 2026); latest pre-release = V4.1.1-QC-V07 (tag Mar 6, 2026; cumulative release notes through June 3, 2026 — same as U113/U114). V4.1.1 stable does not exist; V4.1.1-QC-V08 does not exist. Release notes contain zero entries about FCS, flash write, mutex, SPIC, camera boot, or FlashMemory safety. | LOW |
| ameba-arduino-pro2 issues page (direct WebFetch, 2026-06-13) | **12 open issues; newest = #398 (Mar 29, 2026).** No new issues filed. Issues confirmed: #398, #342, #325, #324, #317, #310, #296, #287, #276, #235, #224, #184. No issue related to FlashMemory, FCS, camera, VOE, or boot failure in open or closed history. Bug entirely unreported on official trackers after **115 research cycles**. | LOW |
| forum.amebaiot.com threads above #4900 (web search sweep, 2026-06-13) | **No new threads indexed above #4900.** Multiple targeted searches for thread IDs 4901–4950 returned zero results from the forum domain. Forum ceiling remains at **#4900** ("Online Conversion — AI model format conversion", first indexed in U113). No new relevant threads in the past 6 hours. | LOW |
| Web-wide error string sweep (7 canary strings, 2026-06-13) | **Zero indexed results — 115 consecutive cycles.** `"FCS KM_status 0x00002081"`, `"It don't do the sensor initial process"`, `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"VOE_OPEN_CMD fail flash"`, `"USE_ISP_RETENTION_DATA"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` — all return zero publicly indexed results on the accessible web. `"ISP_MULTI_FCS_DATA_MAGIC_NUM"` also returns zero. No hardware test result for any proposed workaround posted in any language anywhere on the accessible web. | LOW |
| All Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com/bbs.ai-thinker.com/Bilibili/Gitee, June 13 sweep) | **Zero new technical content — 115th consecutive null cycle.** Chinese-language searches including `"安信可 BW21-CBV RTL8735B 摄像头 FCS flash 冷启动 2026"` return only product pages, unboxing reviews, and unrelated DIY projects. All Chinese community sites remain 403-blocked. No new Chinese-language technical posts or articles about FCS flash-write camera failure on RTL8735B or BW21-CBV found anywhere. | LOW |
| V4.1.1 stable / QC-V08 release search (2026-06-13) | **No release found.** Multiple searches for V4.1.1-QC-V08, stable V4.1.1 release, and June 2026 AmebaPro2 SDK changes return no results. GitHub releases page confirms V4.1.1-QC-V07 is the current latest. No successor published. | LOW |
| RTL8735B FCS / VOE_OPEN_CMD / slot full workaround search (2026-06-13) | **Zero results.** Searches for RTL8735B AmebaPro2 VOE_OPEN_CMD workaround, slot full fix, and FCS camera boot failure workaround return no new publicly accessible results. No developer has publicly documented any hardware test of the three proposed workarounds. | LOW |

**Repository freeze status (as of Cycle U115, June 13, 2026):**

| Repository | Last commit | Days frozen |
|---|---|---|
| ameba-rtos-pro2 main | May 15, 2026 (`3f95070`) | **30 days** |
| ameba-arduino-pro2 dev | June 3, 2026 (`e8dd7e3`) | **10 days** |
| ameba-arduino-pro2 main | Mar 2, 2026 (`93d63514`) | **103 days** ⚠️ |
| ameba-tool-rtos-pro2 | Mar 9, 2026 (`c1d70e7`) | **96 days** |
| ideashatch/HUB-8735 | Dec 2, 2025 (`870a7e0`) | **194 days** |
...
**SDK state as of 2026-06-13 (Cycle U115 — unchanged from U114):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — no fix; FlashMemory.cpp zero mutex calls; V4.1.1-QC-V08 does not exist
- ameba-rtos-pro2: frozen at May 15, 2026 (`3f95070`) — **30 days no change**
- ameba-arduino-pro2: 0 open PRs; 12 open issues, all feature requests; newest = #398 (Mar 29, 2026)
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4900** (unchanged since Cycle U113)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **115th cycle unpatched**
- ameba-arduino-pro2 main frozen: **103 days**

**No confirmed fix. Bug remains unpatched as of 2026-06-13 (Cycle U115).**

**Top unresolved actions (unchanged from U114):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 115 cycles and zero acknowledgment; zero PRs ever filed; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.

## Research Update — 2026-06-13 (Cycle U116)

**Search scope:** Six parallel search threads + direct GitHub fetches: (1) WebFetch — ameba-rtos-pro2 main commits (Ameba-AIoT org); (2) WebFetch — ameba-arduino-pro2 dev commits and releases page (Ameba-AIoT org); (3) WebSearch — English forum/web new threads, camera FCS flash boot failure 2026; (4) WebSearch — error string canary sweep (FCS_I2C_INIT_ERR, FCS_RUN_DATA_NG_KM, USE_ISP_RETENTION_DATA, KM_status 0x2081, FCS_BYPASS); (5) WebSearch — V4.1.1 stable / QC-V08 release; (6) WebSearch — Chinese-language sources (BW21-CBV/RTL8735B/FCS/flash/冷启动).

**Cycle result: NULL — no new actionable findings.** This is the **116th consecutive null cycle**. All research channels confirm the same frozen/blocked state as U115. Both repos still frozen. Forum ceiling unchanged. Error strings return zero results. No confirmed fix. No new workaround test results.

| Source | Key Finding | Priority |
|---|---|---|
| ameba-rtos-pro2 main commits (direct WebFetch, 2026-06-13) | **Confirmed frozen — 29 days.** 10 most recent commits verified identical to U115: `3f95070` (May 15), `afc85a0` (May 15), `9c8b6f6` (May 15), `d2676f1` (May 15), `1c1c8b7` (May 1), `a111e91` (May 1), `7b2b97f` (May 1), `63c0a2f` (May 1), `d54e1a8` (May 1), `687a4c7` (May 1). Zero new commits since May 15, 2026. No flash, FCS, mutex, SPIC, boot, or HAL changes in any observable pipeline. | LOW |
| ameba-arduino-pro2 dev commits (direct WebFetch, 2026-06-13) | **Confirmed frozen — 10 days.** 10 most recent commits verified identical to U115: `e8dd7e3` (Jun 3, "Pre Release Version 4.1.1"), `96cfc51` (Jun 3, PR #411: OV5647+GC4663), `29d47e1` (May 26, PR #410: SPI1), `7db1c7d` (May 19), `3d53672` (May 19, IMX681_5M), `cd0bd40` (May 18, I2C Slave), `13961cc` (May 5). Zero new commits after June 3, 2026. FlashMemory.cpp absent from all recent PRs; zero mutex calls unchanged. | LOW |
| ameba-arduino-pro2 releases page (direct WebFetch, 2026-06-13) | **No new releases.** Latest stable = V4.1.0 (Mar 2, 2026); latest pre-release = V4.1.1-QC-V07 (June 3, 2026 cumulative build). V4.1.1 stable does not exist; V4.1.1-QC-V08 does not exist. Release notes contain zero entries about FCS, flash write, mutex, SPIC, camera boot, or FlashMemory safety. | LOW |
| Forum.amebaiot.com / web-wide camera boot/FCS searches (2026-06-13) | **No new threads above #4900.** Multiple searches for AMB82 camera FCS flash boot failure 2026 return only pre-existing threads (#4302 VOE sensor init, #4316 camera compatibility, #4541 GC4653, #4726 IMX662 ISP tuning). Forum ceiling remains at **#4900** (unchanged since U113). No new forum thread documents the FCS cold-boot or FlashMemory race condition. | LOW |
| Error string canary sweep (6 strings, 2026-06-13) | **Zero indexed results — 116 consecutive cycles.** `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"USE_ISP_RETENTION_DATA"`, `"KM_status 0x00002081"`, `"FCS_BYPASS"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` — all return zero publicly indexed results. No hardware test result for any proposed workaround posted anywhere on the accessible web. | LOW |
| Chinese-language sources (BW21-CBV/RTL8735B 冷启动/flash, 2026-06-13) | **Zero new technical content — 116th consecutive null cycle.** Results limited to product marketing (EEFocus, oshwhub, sekorm, Amazon, hackster.io, mcublog), unboxing posts on bbs.aithinker.com, and one April 2026 Feishu bot integration article. No Chinese-language community content about FCS flash-write camera cold-boot failure anywhere on the accessible web. | LOW |
| V4.1.1 stable / QC-V08 release search (2026-06-13) | **No release found.** Search returns no V4.1.1-QC-V08 or stable V4.1.1 release. Facebook post about V4.1.0 (Mar 2, 2026) is the latest official announcement. V4.1.1-QC-V07 remains the latest known pre-release. | LOW |

**Repository freeze status (as of Cycle U116, June 13, 2026):**
...
**SDK state as of 2026-06-13 (Cycle U116 — unchanged from U115):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — no fix; FlashMemory.cpp zero mutex calls; V4.1.1-QC-V08 does not exist
- ameba-rtos-pro2: frozen at May 15, 2026 (`3f95070`) — **29 days no change**
- ameba-arduino-pro2: 0 open PRs; 12 open issues, all feature requests; newest = #398 (Mar 29, 2026)
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4900** (unchanged since Cycle U113)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **116th cycle unpatched**
- ameba-arduino-pro2 main frozen: **103 days**

**No confirmed fix. Bug remains unpatched as of 2026-06-13 (Cycle U116).**

**Top unresolved actions (unchanged from U115):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 116 cycles and zero acknowledgment; zero PRs ever filed; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.

## Research Update — 2026-06-13 (Cycle U117)

**Search scope:** Eight parallel search threads + direct GitHub/web fetches: (1) WebFetch — ameba-rtos-pro2 commits page (since May 15); (2) WebFetch — ameba-arduino-pro2 dev commits (since June 3), releases page; (3) GitHub MCP — ameba-arduino-pro2 issues created after June 1; (4) WebSearch — forum.amebaiot.com threads above #4900; (5) WebSearch — English error string canary sweep (7 strings); (6) WebSearch — Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com); (7) WebSearch — `device_mutex_lock`/`RT_DEV_LOCK_FLASH` FlashMemory fix references; (8) WebFetch — `ameba-doc-rtos-pro2-sdk.readthedocs-hosted.com/en/latest/application_note/15_ISP.html` accessibility; (9) WebFetch — ameba-arduino-doc commits after June 5.

**Cycle result: NULL — no new actionable findings.** This is the **117th consecutive null cycle**. All research channels confirm the same frozen/blocked state as U116. Both repos still frozen. Forum ceiling unchanged at #4900. Error strings return zero results. No confirmed fix. No new workaround test results.

| Source | Key Finding | Priority |
|---|---|---|
| ameba-rtos-pro2 commits page (direct WebFetch, 2026-06-13) | **Confirmed frozen — 29 days.** 10 most recent commits verified identical to U116: `3f95070` (May 15), `afc85a0` (May 15), `9c8b6f6` (May 15), `d2676f1` (May 15), `1c1c8b7` (May 1), `a111e91` (May 1), `7b2b97f` (May 1), `63c0a2f` (May 1), `d54e1a8` (May 1), `687a4c7` (May 1). Zero new commits since May 15, 2026. No flash, FCS, mutex, SPIC, boot, or HAL changes in any observable pipeline. | LOW |
| ameba-arduino-pro2 dev branch commits (direct WebFetch, 2026-06-13) | **Confirmed frozen — 10 days.** 10 most recent commits verified identical to U116: `e8dd7e3` (Jun 3, "Pre Release Version 4.1.1"), `96cfc51` (Jun 3, PR #411: OV5647+GC4663), `29d47e1` (May 26, PR #410: SPI1), `7db1c7d` (May 19, Pre Release), `3d53672` (May 19, IMX681_5M), `cd0bd40` (May 18, I2C Slave), `13961cc` (May 5). Zero new commits after June 3, 2026. FlashMemory.cpp absent from all recent PRs; zero mutex calls unchanged. | LOW |
| ameba-arduino-pro2 releases page (direct WebFetch, 2026-06-13) | **No new releases.** Stable releases: V4.1.0 (Mar 2, 2026), V4.0.9 (Aug 6, 2025), V4.0.8 (Oct 29, 2024). Latest pre-release: V4.1.1-QC-V07 (tag Mar 6, 2026; cumulative release notes through June 3, 2026). **V4.1.1-QC-V08 does not exist. V4.1.1 stable does not exist.** Release notes contain zero entries about FCS, flash write, mutex, SPIC, camera boot, or FlashMemory safety. | LOW |
| ameba-arduino-pro2 issues created after June 1, 2026 (GitHub MCP `search_issues`, 2026-06-13) | **Zero new issues confirmed via GitHub MCP tool.** Query `repo:Ameba-AIoT/ameba-arduino-pro2 is:issue created:>2026-06-01` returned `total_count: 0`. No issues about FlashMemory, FCS, camera, VOE, or boot failure exist in open or closed history. Bug entirely unreported on official trackers after **117 research cycles**. | LOW |
| ameba-arduino-doc commits (direct WebFetch, 2026-06-13) | **Confirmed frozen — 8 days.** 10 most recent commits: `5a400de` (Jun 5, typo fix), `d42e8a8` (Jun 5, manufacturer/sensor table update), `1726309` (Jun 4, manufacturer names), `f2ee175` (Jun 4, sensor_module_window), `78f2d58` (Jun 4, sensor_module_window), `64863ce` (May 24, I2C Slave examples), `7f0bc9c` (May 21, update application step), `b98a84a` (May 13, TFL examples), `d0b6ca3` (Apr 16, PowerMode documentation), `df6affb` (Apr 2, Installation guide). Zero new commits after June 5, 2026. No FlashMemory, FCS, mutex, camera boot, or ISP flash interaction documentation added. | LOW |
| forum.amebaiot.com threads above #4900 (web search sweep, 2026-06-13) | **No new threads indexed above #4900.** Multiple targeted searches for thread IDs 4901–4920 with filter terms camera/flash/FCS/VOE returned zero results from the forum domain. Forum ceiling remains at **#4900** ("Online Conversion — AI model format conversion", first indexed in U113). Same pre-existing threads (#4302, #4302, #4541, #4748, #4777, #4834, #4865, #4868, #4900) continue to appear in search results without new snippet content. | LOW |
| `ameba-doc-rtos-pro2-sdk.readthedocs-hosted.com/en/latest/application_note/15_ISP.html` (direct WebFetch, 2026-06-13) | **Still HTTP 403 Forbidden.** ISP application note (the most complete FCS documentation page) remains inaccessible without developer authentication. Unchanged since first checked in Cycle U13. No publicly accessible FCS flash-write documentation has appeared in any other URL. | LOW (blocked) |
| Web-wide error string sweep (7 canary strings, 2026-06-13) | **Zero indexed results — 117 consecutive cycles.** `"FCS KM_status 0x00002081"`, `"It don't do the sensor initial process"`, `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"VOE_OPEN_CMD fail flash"`, `"USE_ISP_RETENTION_DATA"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` — all return zero publicly indexed results on the accessible web. `"ISP_MULTI_FCS_DATA_MAGIC_NUM"` also returns zero. No hardware test result for any proposed workaround posted in any language anywhere on the accessible web. | LOW |
| AMB82 `device_mutex_lock` / `RT_DEV_LOCK_FLASH` FlashMemory fix search (2026-06-13) | **Zero results.** Searches for "AMB82 RTL8735B FlashMemory device_mutex_lock RT_DEV_LOCK_FLASH fix 2026" return no forum posts, blog articles, GitHub issues, or PRs discussing the mutex workaround. Confirmed that OV5647/GC4663 sensor support (PR #411, June 3, 2026) did not include any FlashMemory safety changes. | LOW |
| All Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com/bbs.ai-thinker.com/Bilibili/Gitee, June 13 sweep) | **Zero new technical content — 117th consecutive null cycle.** Chinese-language searches for BW21-CBV/RTL8735B/FCS/flash/冷启动/故障 return only product marketing pages (EEFocus, sekorm, xcc.com), unrelated DIY projects (mcublog.cn Feishu bot), and bbs.aithinker.com unboxing threads. All Chinese community sites remain 403-blocked. No new Chinese-language technical posts or articles about FCS flash-write camera failure on RTL8735B or BW21-CBV found anywhere. | LOW |

**Repository freeze status (as of Cycle U117, June 13, 2026):**
...
**SDK state as of 2026-06-13 (Cycle U117 — unchanged from U116):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — no fix; FlashMemory.cpp zero mutex calls; V4.1.1-QC-V08 does not exist
- ameba-rtos-pro2: frozen at May 15, 2026 (`3f95070`) — **29 days no change** (confirmed by commits page WebFetch)
- ameba-arduino-pro2: 0 open PRs; 12 open issues, all feature requests; zero new issues after June 1 (confirmed by GitHub MCP)
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4900** (unchanged since Cycle U113)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **117th cycle unpatched**
- ameba-arduino-pro2 main frozen: **103 days**

**No confirmed fix. Bug remains unpatched as of 2026-06-13 (Cycle U117).**

**Top unresolved actions (unchanged from U116):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 117 cycles and zero acknowledgment; zero PRs ever filed; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.

---

## Research Update — 2026-06-14 (Cycle U118)

**Search window:** 2026-06-13 (last cycle) → 2026-06-14 (this cycle). One-day delta check.

**Summary:** Complete null cycle. No new commits, releases, issues, forum posts, or public content of any kind related to the FCS flash→camera race condition. Both SDK repositories remain frozen at the same SHAs recorded in U117. Bug unpatched for 118 consecutive research cycles.

### Sources checked (2026-06-14)

| Source | Finding | Priority |
|---|---|---|
| ameba-arduino-pro2 dev branch commits (WebFetch, 2026-06-14) | **Confirmed still frozen at June 3, 2026 (`e8dd7e3`, "Pre Release Version 4.1.1").** No new commits in the 11 days since V4.1.1-QC-V07 tag. FlashMemory.cpp untouched; zero mutex calls unchanged. | LOW |
| ameba-rtos-pro2 main commits (WebFetch, 2026-06-14) | **Confirmed still frozen at May 15, 2026 (`3f95070`, "Sync upstream").** Now **30 days** without a commit. No flash, FCS, SPIC, mutex, or VOE changes in any observable pipeline. | LOW |
| ameba-arduino-pro2 releases (web search, 2026-06-14) | **No new releases.** V4.1.1-QC-V07 (June 3, 2026) remains the latest pre-release. V4.1.1 stable does not exist. V4.1.0 (Mar 2, 2026) remains latest stable. No FlashMemory mutex fix in any release. | LOW |
| forum.amebaiot.com (web search, 2026-06-14) | **Zero new threads above #4900.** June-specific query returned no FCS/flash/camera content from the forum. Pre-existing threads (#4302, #4834, etc.) only. Forum ceiling remains #4900. | LOW |
| Web-wide canary string sweep (web search, 2026-06-14) | **Zero indexed results — 118th consecutive null cycle.** `"FCS KM_status 0x00002081"`, `"It don't do the sensor initial process"`, `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"USE_ISP_RETENTION_DATA"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` all return zero publicly indexed results. No hardware test result for any proposed workaround found anywhere. | LOW |
| AMB82 FlashMemory mutex fix search (web search, 2026-06-14) | **Zero results.** Targeted search for "AMB82 RTL8735B FlashMemory mutex fix camera boot June 2026" returned no forum posts, GitHub issues, PRs, or articles. Bug remains entirely undocumented outside this research log after 118 cycles. | LOW |
| Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com, 2026-06-14) | **Zero new content — 118th consecutive null cycle.** No Chinese-language technical posts about BW21-CBV/RTL8735B/FCS/冷启动/flash camera failure found in any accessible source. | LOW |

**Repository freeze status (as of Cycle U118, June 14, 2026):**

| Repository | Last commit | Days frozen |
|---|---|---|
| ameba-rtos-pro2 main | May 15, 2026 (`3f95070`) | **30 days** |
| ameba-arduino-pro2 dev | June 3, 2026 (`e8dd7e3`) | **11 days** |
| ameba-arduino-pro2 main | Mar 2, 2026 (`93d63514`) | **104 days** ⚠️ |
| ameba-tool-rtos-pro2 | Mar 9, 2026 (`c1d70e7`) | **97 days** |
| ideashatch/HUB-8735 | Dec 2, 2025 (`870a7e0`) | **194 days** |
| ameba-arduino-doc | June 5, 2026 (`5a400de`) | **9 days** (editorial only) |

**SDK state as of 2026-06-14 (Cycle U118 — unchanged from U117):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — no fix; FlashMemory.cpp zero mutex calls; no V4.1.1-QC-V08
- ameba-rtos-pro2: frozen at May 15, 2026 (`3f95070`) — **30 days no change**
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4900** (unchanged since Cycle U113)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **118th cycle unpatched**

**No confirmed fix. Bug remains unpatched as of 2026-06-14 (Cycle U118).**

**Top unresolved actions (unchanged from U117):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 118 cycles and zero acknowledgment; zero PRs ever filed; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.

## Research Update — 2026-06-14 (Cycle U119)

**Search window:** 2026-06-14 (same day, second cycle). Parallel web searches across GitHub repos, English forum/web, Chinese sources, and June 3 build content analysis.

**Summary:** Essentially another null cycle. Both SDK repositories remain frozen at the same SHAs as U118. No new forum threads, hardware tests, or Chinese-language content found. Two minor new details: the June 3 V4.1.1-QC-V07 build content is now confirmed via commit `96cfc51` (adds OV5647 + GC4663 sensors, tools v1.4.12, new `fcs_data_gc4663.bin`; FlashMemory.cpp NOT changed). The ameba-rtos-pro2 freeze milestone reaches **31 days** with no activity. Bug unpatched for 119 consecutive research cycles.

### Sources checked (2026-06-14, Cycle U119)

| Source | Finding | Priority |
|---|---|---|
| ameba-arduino-pro2 dev branch commits (WebFetch, 2026-06-14) | **Confirmed frozen at June 3, 2026.** Top 10 commits retrieved: `e8dd7e3` (June 3, "Pre Release Version 4.1.1"), `96cfc51` (June 3, "Update Code base and add cam OV5647, GC4663 (#411)"), `29d47e1` (May 26, "Update SPI API for SPI1 switching (#410)"), `7db1c7d` (May 19, "Pre Release Version 4.1.1"), `3d53672` (May 19, "Update Code base and add cam IMX681_5M (#409)"). Zero new commits after June 3, 2026. | LOW |
| ameba-arduino-pro2 commit `96cfc51` "Update Code base and add cam OV5647, GC4663 (#411)" (June 3, 2026) — content analysis | **PR #411 detail confirmed from commit diff.** Files changed include: `fcs_data_gc4663.bin` (NEW — GC4663 camera now has FCS support), `sensor_gc4663.bin`, `sensor_ov5647.bin`, `iq_gc4663.bin`, ISP IQ JSON configs, `boards.txt`, `amb_ard_user_sensor_list.h`, `Wire.cpp`, `Wire.h`, `build_info.h`, `sensor.h`, multiple `.a` library files; tools bumped to **v1.4.12** (from 1.4.11). **`FlashMemory.cpp`, `device_lock.h`, `video_api.h` NOT in the changed-file list.** No FCS cold-boot fix, no mutex addition. | LOW (confirms no fix) |
| ameba-rtos-pro2 main commits (WebFetch, 2026-06-14) | **Confirmed frozen at May 15, 2026 (`3f95070`).** Now **31 days** without a commit — milestone. No flash, FCS, SPIC, mutex, or VOE changes in any observable pipeline. | LOW |
| ameba-arduino-pro2 releases (WebFetch, 2026-06-14) | **No new releases.** Latest stable = V4.1.0 (Mar 2, 2026). Latest pre-release = V4.1.1-QC-V07 (tag date: Mar 6, 2026; build date: June 3, 2026). V4.1.1 stable does not exist. No V4.1.1-QC-V08. | LOW |
| ameba-arduino-pro2 open issues (WebFetch, 2026-06-14) | **No new issues.** Newest issue remains #398 (Mar 29, 2026). No issues about FlashMemory, FCS, camera boot failure, VOE_OPEN_CMD, or device_mutex_lock filed in open or closed history. | LOW |
| forum.amebaiot.com/t/does-enabling-secure-boot-require-access-to-the-nda-sdk/4841 (Google-indexed, ~May 2026) | **New thread identified above prior #4840 ceiling.** Title: "Does enabling secure boot require access to the NDA SDK?" — tangentially relevant since the RTL8735B FCS boot sequence runs in the TrustZone-protected secure domain, and the KM co-processor firmware (voe.bin) is the closed-source binary at the heart of our bug. Content 403-blocked; no resolution extractable. If an NDA SDK provides secure-domain source access, it might expose the KM's I2C initialization code (`FCS_I2C_INIT_ERR` origin). | LOW (blocked) |
| Web-wide canary string sweep (web search, 2026-06-14) | **Zero indexed results — 119th consecutive null cycle.** `"FCS KM_status 0x00002081"`, `"It don't do the sensor initial process"`, `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"USE_ISP_RETENTION_DATA"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` all return zero publicly indexed results. No hardware test result for any proposed workaround found anywhere. | LOW |
| Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com, 2026-06-14) | **Zero new content — 119th consecutive null cycle.** No Chinese-language technical posts about BW21-CBV/RTL8735B/FCS/冷启动/flash camera failure found in any accessible source. All Chinese community sites remain 403-blocked. | LOW |

**Repository freeze status (as of Cycle U119, June 14, 2026):**

| Repository | Last commit | Days frozen |
|---|---|---|
| ameba-rtos-pro2 main | May 15, 2026 (`3f95070`) | **31 days** |
| ameba-arduino-pro2 dev | June 3, 2026 (`e8dd7e3`/`96cfc51`) | **11 days** |
| ameba-arduino-pro2 main | Mar 2, 2026 (`93d63514`) | **104 days** ⚠️ |
| ideashatch/HUB-8735 | Dec 2, 2025 (`870a7e0`) | **194 days** |

**SDK state as of 2026-06-14 (Cycle U119 — unchanged from U118):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — **confirmed no fix**; FlashMemory.cpp absent from PR #411 diff; tools now v1.4.12; GC4663 + OV5647 sensors added
- ameba-rtos-pro2: frozen at May 15, 2026 (`3f95070`) — **31 days no change**
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4841** newly confirmed (above prior #4840 ceiling, below documented #4900)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **119th cycle unpatched**

**No confirmed fix. Bug remains unpatched as of 2026-06-14 (Cycle U119).**

**Top unresolved actions (unchanged from U118):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 119 cycles and zero acknowledgment; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.

## Research Update — 2026-06-14 (Cycle U120)

**Search scope:** Four parallel primary agents + multiple sub-agents: (1) GitHub — ameba-rtos-pro2 and ameba-arduino-pro2 commits/releases/issues/PRs post-May 22; (2) English forum/web — new threads above #4841/#4900, FCS Disable / `device_mutex_lock` / `USE_ISP_RETENTION_DATA` hardware test reports; (3) Chinese sources — CSDN/知乎/EEWorld/21IC/bbs.aithinker.com/bbs.ai-thinker.com/Bilibili/Gitee; (4) GitHub code search — FlashMemory.cpp mutex state, PRs, VOE release notes beyond 1.7.1.0.

**Cycle result:** Essentially null — 120th consecutive cycle with zero confirmed fix or hardware test result. One new secondary finding: **ameba-rtos-pro2 V1.0.3 RTOS SDK release** (May 22, 2026) confirmed, not previously documented in this log. Forum thread #4641 newly catalogued. Multiple sub-agents hit API rate limits; all channels that did respond confirmed the frozen/blocked state documented in U117–U119.

| Source | Key Finding | Priority |
|---|---|---|
| ameba-rtos-pro2 GitHub Releases page (WebFetch, 2026-06-14) | **ameba-rtos-pro2 V1.0.3 RTOS SDK release confirmed** — dated May 22, 2026. Release notes cover: RTSP source code patch, WebSocket viewer patch. **No flash, FCS, SPIC mutex, boot ROM, or sensor-init changes.** This is the first documented numbered release of the RTOS SDK (distinct from the Arduino SDK pre-releases). The repo was previously tracked only via commit SHAs; V1.0.3 release was not catalogued in U117–U119. The existence of a named release on May 22 — the same day the Arduino dev branch diverged — is consistent with a concurrent SDK synchronization event. | LOW (new catalog entry, no fix) |
| forum.amebaiot.com/t/spi1-miso-issue-on-amb82-mini/4641 (search engine snippet, 2026-06-14) | **Thread #4641 "SPI1 MISO Issue on AMB82 MINI" newly catalogued** (previously untracked). User reports SPI1 MISO (Pin 0 / PF5) always reads 0x00 regardless of MOSI data — SPI1 hardware communication with peripherals fails. Addressed in V4.1.0 ("Update SPI API for SPI1 switching") and continued in PR #410 ("Update SPI API for SPI1 switching", merged May 26, 2026). SPI1 is the secondary SPI bus used for external peripherals, distinct from the internal SPI NOR flash bus (SPIC). Not related to our FCS flash-write camera boot bug. Content 403-blocked. | LOW (catalog, unrelated) |
| blog.csdn.net/pythonsys/article/details/149068179 (search snippet, 2026-06-14) | **New CSDN article newly surfaced** — title appears to be about AMB82 / RTL8735B brick recovery or firmware update procedure. Article 149068179 was not indexed in prior Chinese-language sweeps. Content 403-blocked on direct fetch. Not confirmed as relevant to FCS flash-write camera failure. Adds to the CSDN catalog for RTL8735B. | LOW (catalog, blocked) |
| ameba-rtos-pro2 commits page (direct WebFetch agents, 2026-06-14) | **Confirmed frozen — 30+ days.** Multiple independent agents confirm HEAD remains `3f95070` (May 15, 2026). Zero new commits since May 15. PR #17 (ethernet USB buffer overflow security fix) still open and unmerged. No flash, FCS, SPIC, mutex, VOE, or boot changes in any observable pipeline. | LOW |
| ameba-arduino-pro2 dev branch commits and releases (direct WebFetch agents, 2026-06-14) | **Confirmed frozen at June 3, 2026** — dev HEAD: `e8dd7e3` (Pre Release Version 4.1.1), parent: `96cfc51` (PR #411: OV5647+GC4663, 107 files). Zero new commits after June 3. V4.1.1-QC-V07 remains latest pre-release; V4.1.1 stable does not exist. FlashMemory.cpp: absent from PR #411 file diff; zero mutex calls confirmed unchanged. `video_api.h` `USE_ISP_RETENTION_DATA` still commented out. | LOW |
| Web-wide English search: all key error strings (2026-06-14) | **Zero indexed results — 120th consecutive null cycle.** `"FCS KM_status 0x00002081"`, `"It don't do the sensor initial process"`, `"FCS_I2C_INIT_ERR"`, `"FCS_RUN_DATA_NG_KM"`, `"VOE_OPEN_CMD fail flash"`, `"USE_ISP_RETENTION_DATA"`, `"device_mutex_lock RT_DEV_LOCK_FLASH"` — all return zero publicly indexed results on the accessible web. No hardware test result for any proposed workaround has been posted anywhere in any language. `device_mutex_lock` + RTL8735B/AMB82: zero results across Reddit, Hackster.io, Arduino forum, Realtek forum, and GitHub. | LOW |
| All Chinese-language sources (CSDN/知乎/EEWorld/21IC/bbs.aithinker.com/bbs.ai-thinker.com/Bilibili/Gitee, June 14 sweep) | **Zero new technical content — 120th consecutive null cycle.** All Chinese community sites remain 403-blocked or return zero relevant results. CSDN article 149068179 newly identified but 403-blocked. bbs.aithinker.com, bbs.ai-thinker.com: all BW21-CBV threads remain inaccessible. No new Chinese-language forum posts or articles about FCS flash-write camera failure on RTL8735B or BW21-CBV found anywhere. | LOW |
| Multiple sub-agents hit API rate limits (2026-06-14) | **Search infrastructure saturation.** Approximately 15 sub-agents hit "API Error: Server is temporarily limiting requests" before completing their search tasks. This is consistent with 120 consecutive research cycles saturating publicly indexed content — the same queries have been executed ~120 times against the same static sources. No missed findings expected; the rate-limited agents' targets were covered by agents that did complete successfully. | LOW (infrastructure note) |

**ameba-rtos-pro2 V1.0.3 — significance assessment:**

The V1.0.3 release (May 22, 2026) is a numbered RTOS SDK release, distinct from the Arduino SDK pre-releases tracked in prior cycles. Its changelog:
- RTSP source code patch: likely adds or fixes RTSP streaming functionality in user-accessible source
- WebSocket viewer patch: adds or fixes WebSocket-based debugging/viewing tool
- **No flash, FCS, mutex, or camera boot content**

The release appears to be a documentation/tooling release rather than a binary flash/VOE change. The same date as the last Arduino SDK activity (May 22 was the prior research cutoff) suggests a coordinated SDK activity event, now confirmed as containing no fix for our bug.

**Repository freeze status (as of Cycle U120, June 14, 2026):**
...
| ameba-arduino-pro2 main | Mar 2, 2026 (`93d63514`) | **104 days** ⚠️ |
| ameba-tool-rtos-pro2 | Mar 9, 2026 (`c1d70e7`) | **97 days** |
| ideashatch/HUB-8735 | Dec 2, 2025 (`870a7e0`) | **195 days** |

**SDK state as of 2026-06-14 (Cycle U120 — unchanged from U119):**
- Latest stable Arduino SDK: V4.1.0 (Mar 2, 2026) — no fix
- Latest pre-release Arduino SDK: V4.1.1-QC-V07 (build20260603, June 3, 2026) — no fix; OV5647 + GC4663 sensors; tools v1.4.12; FlashMemory.cpp unchanged
- ameba-rtos-pro2 main: frozen at May 15, 2026 (`3f95070`) — 30 days; **V1.0.3 release (May 22, 2026) confirmed — RTSP/WebSocket patches only, no FCS/flash fix**
- VOE binary: last confirmed v1.7.1.0 (synced May 1, 2026, `d54e1a8`)
- Forum ceiling: **#4841** (as confirmed in U119)
- FlashMemory.cpp: last commit `4fdfbec` (Sep 30, 2025), zero mutex calls — **120th cycle unpatched**
- ameba-arduino-pro2 main frozen: **104 days**

**No confirmed fix. Bug remains unpatched as of 2026-06-14 (Cycle U120).**

**Top unresolved actions (unchanged from U119):**
1. **Hardware test of "Camera FCS Mode = Disable"** — full source-code chain confirmed across 3 files (postbuild.cpp + video_boot.c + video_api.c); dummy blob → invalid MFCS magic → KM bypass (0x0083) → camera re-init via application layer. No public hardware test result exists anywhere. **Highest priority.**
2. **Hardware test of `device_mutex_lock(RT_DEV_LOCK_FLASH)` wrapper** — Realtek's own `flash/src/main.c` demonstrates the required pattern; 16+ RTOS SDK files use it correctly; FlashMemory.cpp confirmed sole exception. Callable from Arduino: `extern "C" { void device_mutex_lock(unsigned int); void device_mutex_unlock(unsigned int); } #define RT_DEV_LOCK_FLASH 1`.
3. **File a GitHub Issue on ameba-arduino-pro2** — bug entirely undocumented outside this research log; 120 cycles and zero acknowledgment; filing would be the first public disclosure.
4. **Hardware test of `USE_ISP_RETENTION_DATA`** — eliminates ISP competing SPIC writes entirely; requires uncommenting `// #define USE_ISP_RETENTION_DATA` in `video_api.h`.
