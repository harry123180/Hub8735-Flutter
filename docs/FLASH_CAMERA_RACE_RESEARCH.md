...
## Research Update — 2026-05-10

### Finding 134 — Both Repos Confirmed Static Since Last Cycle; No New Commits (May 10, 2026)
**Source:** Direct compare fetches (2026-05-10)
https://github.com/Ameba-AIoT/ameba-arduino-pro2/compare/13961cc...dev
https://github.com/Ameba-AIoT/ameba-rtos-pro2/compare/1c1c8b7...main
**Priority:** LOW — Status confirmation; no new fix commits

GitHub compare endpoints confirm both repositories are identical to their last documented commits:
...
- `13961cc...dev` → "13961cc and dev are identical."
- `1c1c8b7...main` → "1c1c8b7 and main are identical."

Zero new commits to either repository since 2026-05-07 (last research cycle). No new releases, no new pull requests on `ameba-arduino-pro2`. `FlashMemory.cpp` has not been modified since SHA `4fdfbec` (September 30, 2025) — now **over 8.5 months** without change. `video_api.c` unchanged since March 3, 2026.

---

### Finding 135 — FlashMemory.cpp Re-Confirmed Unpatched via Direct Raw Fetch (May 10, 2026)
**Source:** https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp
**Priority:** LOW — Direct re-verification; bug confirmed still present

Fresh raw fetch of `FlashMemory.cpp` (dev branch) confirms the file is unchanged from SHA `4fdfbec` (September 30, 2025):
- `write()` function: calls `flash_erase_sector()` (loop over all 48 sectors at `0xFD0000–0xFFFFFF`) and `flash_stream_write()` with **zero** `device_mutex_lock` / `device_mutex_unlock` calls
- `writeWord()` function: calls `flash_write_word()` and, on bit-flip conflict, falls through to `flash_erase_sector()` / `flash_stream_write()` — also **zero** mutex calls
- Zero occurrences of `device_mutex_lock`, `device_mutex_unlock`, `RT_DEV_LOCK_FLASH`, or `device_lock.h` anywhere in the file

Bug confirmed unpatched for the 6th consecutive 6-hour cycle without a fix.

---

### Finding 136 — No New SDK Release; V4.1.1 Stable Still HTTP 404 (May 10, 2026)
**Source:** https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases (fetched 2026-05-10)
https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases/tag/V4.1.1
**Priority:** LOW — Status confirmation; no untracked release containing a fix

- **V4.1.1 stable release tag**: HTTP 404 — still does not exist.
- **V4.1.1-QC-V05** (internal build April 30, 2026) remains the latest pre-release.
- **V4.1.0** (March 2, 2026) remains the latest stable release.
- No new QC tag (V4.1.1-QC-V06 or similar) has been published.

A complete scan of all V4.1.1-QC-V05 changelog entries (2026/03/06 through 2026/04/30) confirms zero mentions of FlashMemory mutex, RT_DEV_LOCK_FLASH, FCS flash write protection, camera boot failure, or concurrent flash access.

---

### Finding 137 — No New GitHub Issues Above #398; Issues #408–#410 Confirmed HTTP 404 (May 10, 2026)
**Source:** https://github.com/Ameba-AIoT/ameba-arduino-pro2/issues (fetched 2026-05-10)
https://github.com/Ameba-AIoT/ameba-arduino-pro2/issues/408
https://github.com/Ameba-AIoT/ameba-arduino-pro2/issues/409
**Priority:** LOW — Status confirmation; no new bug reports

- **ameba-arduino-pro2**: Issues #408, #409 return HTTP 404 — they do not exist. Highest filed issue remains #398 (March 29, 2026). 12 confirmed open issues; none relate to FlashMemory, FCS, camera boot failure, VOE errors, or flash-camera interaction.
- **ameba-rtos-pro2**: 3 open issues (highest: #16, January 2026). Issue #17 returns HTTP 404.
- **ideashatch/HUB-8735**: Only issue #10 (August 2025 — PS5268 sensor ID, unrelated). No new issues.
- **ameba-arduino-pro2 PRs**: 0 open, 319 closed. No FlashMemory/FCS/mutex PR in history. The only closed PR matching "FlashMemory" search is PR #333 "Update WiFi example" (September 9, 2025) — unrelated.

---

### Finding 138 — Forum Thread #4802 Newly Identified: USB Modem CDC ECM Issue; Unrelated
**Source:** Google search result; forum.amebaiot.com thread #4802 (403-blocked)
...
Thread #4802 — "AMB82-Mini USB Host CDC ECM fails to enumerate Quectel EC200U 4G modem — 'ecm init fail'" — surfaced in search results for this cycle. It is a USB peripheral initialization problem for a 4G modem and has no relation to FlashMemory, FCS, camera cold-boot failure, or the flash concurrency race condition. Content is HTTP 403-blocked; title only available from search-engine snippets.

This fills in a gap: the previously documented thread sequence was #4803 (USB Ethernet failing), and #4802 was unlogged. Both are USB-related, unrelated to our bug.

---

### Finding 139 — ameba-rtos-pro2 Tags: No New Tags Since V1.0.3-aiglass.07 (April 2, 2026)
**Source:** https://github.com/Ameba-AIoT/ameba-rtos-pro2/tags (fetched 2026-05-10)
**Priority:** LOW — Status confirmation

The `ameba-rtos-pro2` tag list remains unchanged since Finding 116 (documented May 6, 2026). The five most recent tags are:
...
No new tags have been created since V1.0.3-aiglass.07 (April 2, 2026). No new "main SDK" tag (e.g., V4.1.1 or similar) exists.

---

### Finding 140 — ameba-arduino-doc: No New Commits; No FCS/FlashMemory Warning Added (May 10, 2026)
**Source:** https://github.com/Ameba-AIoT/ameba-arduino-doc/commits/main (fetched 2026-05-10)
**Priority:** LOW — Documentation status unchanged

The official Arduino documentation repository (`ameba-arduino-doc`) has no new commits since SHA `d0b6ca3` ("Update PowerMode documentation", April 16, 2026). The five most recent commits remain identical to those documented in Finding 118 (May 6, 2026). No commit mentions FlashMemory, FCS, camera boot failure, mutex, or any flash-camera interaction. The bug remains completely absent from all official Realtek/Ameba documentation.

---

### Finding 141 — Forum Threads #4841–#4860 Return No Google-Indexed Results; Bug Strings Still Zero
**Source:** Google site-search and targeted searches (2026-05-10)
**Priority:** LOW — Status confirmation

Targeted searches for forum threads above #4840 return zero results from Google's index. The following bug-signature string searches all return zero new results:
...
| `"VOE_OPEN_CMD" fail ameba` | **Zero results** |
| `"device_mutex_lock" "FlashMemory" Ameba` | **Zero results** |
| `RTL8735B AMB82 FlashMemory FCS camera fix mutex 2026` | Zero relevant hits |
| `AMB82 FCS flash 摄像头 启动失败 CSDN 2026` (Chinese) | **Zero relevant results** |
| `BW21-CBV FCS camera boot flash 2026` | Zero results |

The bug's specific error signatures remain completely unindexed on the public internet. Zero new Chinese-language reports across CSDN, Zhihu, 21ic, EEWorld, bbs.ai-thinker.com. The root cause (FlashMemory.cpp bypasses RT_DEV_LOCK_FLASH) is documented exclusively in this research log.

---

### Complete Status Sweep — 2026-05-10

| Repository / Source | Last activity | Status |
|---|---|---|
| ameba-arduino-pro2 (dev branch) | May 5, 2026 — SHA `13961cc` (AMB82-zero SWD) | **No new commits — compare endpoint confirms identical** |
| ameba-arduino-pro2 (releases) | V4.1.1-QC-V05 (Apr 30, 2026); V4.1.1 stable = HTTP 404 | **No new release** |
| ameba-rtos-pro2 (main branch) | May 1, 2026 — SHA `1c1c8b7` (WLAN dhcp sync); HEAD identical | **No new commits; no fix** |
| ameba-rtos-pro2 (tags) | V1.0.3-aiglass.07 (Apr 2, 2026) — most recent | **No new tags since Apr 2, 2026** |
...
| Ai-Thinker-Open GitHub org | — | **No BW21-CBV repository** |
| ameba-arduino-pro2 forks (36 total) | Various | **Zero forks contain FlashMemory mutex patch** |
| forum.amebaiot.com | Thread #4802 (USB CDC ECM, unrelated) newly logged; all 403-blocked; threads #4841+ not Google-indexed | **No new FCS/flash/camera threads** |
| CSDN / Zhihu / 21ic / EEWorld | — | **Zero Chinese-language reports — reconfirmed** |
| bbs.ai-thinker.com (BW21-CBV) | tid=47223 (DIY digital camera) highest confirmed | **No camera/FCS bug threads** |
| FlashMemory.cpp (dev, SHA `4fdfbec`) | Sept 30, 2025 (>8.5 months without change) | **Still NO mutex fix — confirmed by direct raw fetch** |
| video_api.c (main) | March 3, 2026 | **Unguarded ftl_common_write() call; no mutex fix — confirmed** |
| Official documentation (ameba-arduino-doc) | April 16, 2026 — SHA `d0b6ca3` | **No new commits; no FlashMemory/FCS warning** |
| Public web (all bug-signature strings) | — | **Zero new indexed results — root cause uniquely documented in this log** |

**No HIGH priority confirmed fix found. Bug status: publicly undocumented and unpatched as of 2026-05-10.**

---

### Sources Added (Update 2026-05-10)
...
- ameba-arduino-pro2 FlashMemory.cpp dev (re-confirmed no mutex, SHA `4fdfbec`, >8.5 months unmodified): https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp
- ameba-arduino-pro2 releases (V4.1.1-QC-V05 latest; V4.1.1 stable = HTTP 404; no new tag): https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases
- ameba-arduino-pro2 release tag V4.1.1 (HTTP 404 confirmed): https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases/tag/V4.1.1
- ameba-arduino-pro2 issues (12 open; highest: #398 Mar 2026; #408–#409 = HTTP 404): https://github.com/Ameba-AIoT/ameba-arduino-pro2/issues
- ameba-rtos-pro2 issues (3 open; highest: #16 Jan 2026; #17 = HTTP 404): https://github.com/Ameba-AIoT/ameba-rtos-pro2/issues
- ameba-rtos-pro2 tags (V1.0.3-aiglass.07 Apr 2, 2026 — most recent; no new tags): https://github.com/Ameba-AIoT/ameba-rtos-pro2/tags
- ameba-arduino-doc commits/main (re-confirmed last: `d0b6ca3`, Apr 16, 2026; no new commits): https://github.com/Ameba-AIoT/ameba-arduino-doc/commits/main
- forum.amebaiot.com thread #4802 (AMB82-Mini USB CDC ECM / Quectel EC200U; unrelated; 403-blocked): https://forum.amebaiot.com/t/amb82-mini-usb-host-cdc-ecm-fails-to-enumerate-quectel-ec200u-4g-modem-ecm-init-fail/4802
- ameba-arduino-pro2 PR search FlashMemory closed (PR #333 only match — WiFi example, unrelated): https://github.com/Ameba-AIoT/ameba-arduino-pro2/pulls?q=is%3Apr+FlashMemory+is%3Aclosed
...
### Finding 156 — Both Repositories Confirmed Static Since Previous Cycle; No New Commits
**Source:** Direct compare fetches + commit page fetches (2026-05-10)
https://github.com/Ameba-AIoT/ameba-arduino-pro2/compare/13961cc...dev
https://github.com/Ameba-AIoT/ameba-rtos-pro2/compare/1c1c8b7...main
**Priority:** LOW — Status confirmation; no new fix commits

Both GitHub compare endpoints return "identical" — no new commits since previously documented activity:
...
`FlashMemory.cpp` (SHA `4fdfbec`) remains unchanged since September 30, 2025 — now over **9 months** without modification. `video_api.c` unchanged since March 3, 2026. No new releases, PRs, or issues on either repository.

---

### Finding 157 — FlashMemory.cpp write() Re-Confirmed Unpatched via Direct Raw Fetch (May 10, 2026)
**Source:** https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp
**Priority:** LOW — Direct re-verification; bug confirmed still present

Fresh raw fetch confirms `write()` and `writeWord()` are unchanged from SHA `4fdfbec`:
- Zero calls to `device_mutex_lock` or `device_mutex_unlock`
- Zero `#include "device_lock.h"` or `RT_DEV_LOCK_FLASH` references
- `write()` calls `flash_erase_sector()` (N times) + `flash_stream_write()` with no serialization
- `writeWord()` falls through to `flash_erase_sector()` + `flash_stream_write()` on bit-flip conflict with no serialization

The six unguarded raw flash operations documented in Finding 137 remain present and unpatched.

---

### Finding 158 — No New Issues, PRs, or Releases; ameba-arduino-pro2 Issues Confirmed 12 Open
**Source:** GitHub issues, PRs, and releases pages (2026-05-10)
**Priority:** LOW — Status confirmation

- **ameba-arduino-pro2 issues**: 12 open; highest filed #398 (Mar 29, 2026 — raw video feature request). Issues #408, #409, #410 return HTTP 404. **Zero new issues related to FlashMemory, FCS, camera boot failure, VOE errors, or flash-camera interaction.**
- **ameba-rtos-pro2 issues**: 3 open; highest #16 (Jan 27, 2026). Issue #17 returns HTTP 404. **Zero new relevant issues.**
- **ideashatch/HUB-8735**: Only issue #10 (Aug 2025 — PS5268 sensor ID fail, unrelated). No new issues.
- **ameba-arduino-pro2 PRs**: 0 open; 319 closed. No FlashMemory/FCS/mutex PR has ever been filed in the complete PR history.
- **Releases**: V4.1.1-QC-V05 (Apr 30, 2026) remains latest. V4.1.1 stable = HTTP 404. V4.1.2 = HTTP 404.

---

### Finding 159 — Forum Threads Above #4847 Not Yet Indexed; No New Bug-String Matches (May 10, 2026)
**Source:** Google site-search + targeted URL probes for forum.amebaiot.com (2026-05-10)
**Priority:** LOW — Status confirmation

Direct URL probes of threads #4848–#4870 all return HTTP 403. No threads in this range have been indexed by Google with content matching any of the bug's signature strings (`VOE_OPEN_CMD`, `FCS KM_status`, `slot full`, `sensor initial process`, `FlashMemory`). The highest confirmed indexed forum thread remains **#4847** ("I2C1 for MPU6050", ~May 6–7, 2026, documented in Finding 139). No new forum posts describing the FlashMemory/FCS race condition or its symptoms have appeared.

A review of previously identified adjacent threads returned no new information:
- Thread #4777 ("AMB82-Mini onboard camera sensor identification and VOE setup") — still 403-blocked; no new snippet visible
- Thread #4302 ("[VOE]frame_end sensor didn't initialize done") — content still 403-blocked; symptom is a different (non-flash) sensor timing issue

---

### Finding 160 — ameba-arduino-doc and All Documentation Sources: No New Commits (May 10, 2026)
**Source:** https://github.com/Ameba-AIoT/ameba-arduino-doc/commits/main (fetched 2026-05-10)
**Priority:** LOW — Documentation status unchanged

The official Arduino documentation repository (`ameba-arduino-doc`) has **no commits** after SHA `d0b6ca3` ("Update PowerMode documentation", April 16, 2026). The hosted documentation at `ameba-doc-arduino-sdk.readthedocs-hosted.com` Flash Memory page was re-confirmed to contain no warning, caution, or known-issue note about:
- FlashMemory API use during active camera/FCS sessions
- Concurrent flash write hazards
- Required precautions when FCS mode is enabled

The bug is absent from all three official Realtek/Ameba documentation channels (GitHub docs repo, readthedocs hosted, amebaiot.com web docs).

---

### Finding 161 — All Bug-Signature Strings Continue to Return Zero Results (May 10, 2026)
**Source:** Web searches (Google indexed, 2026-05-10) — English and Chinese
**Priority:** LOW — Confirms continued public ignorance of the bug
...
| `"VOE_OPEN_CMD fail" ameba` | **Zero results** |
| `"device_mutex_lock" "FlashMemory" Ameba` | **Zero results** |
| `RTL8735B AMB82 FlashMemory FCS camera mutex fix 2026` | Zero relevant hits |
| `ameba-arduino-pro2 ameba-rtos-pro2 FlashMemory mutex device_mutex_lock fix patch 2026` | Zero results |
| `AMB82 RTL8735B flash camera FCS VOE site:csdn.net OR site:zhihu.com OR site:21ic.com` | Zero relevant hits (same prior CSDN articles only) |
| `BW21-CBV flash camera FCS cold boot fail` (Chinese + English) | **Zero results** |
| `site:forum.amebaiot.com camera flash FCS 2026` | Returns only thread #4302 (different bug, FCS mode *disabled*) |

The root cause analysis documented in this log (Hypothesis F — `FlashMemory.cpp` bypasses `RT_DEV_LOCK_FLASH`, causing concurrent SPI commands to abort the FCS page-program at `0xF0D000`) remains the only public record of this analysis anywhere on the indexed internet as of 2026-05-10.
...
| Repository / Source | Last activity | Status |
|---|---|---|
| ameba-arduino-pro2 (dev branch) | May 5, 2026 — SHA `13961cc` (AMB82-zero SWD logic) | **No new commits — compare `13961cc...dev` = identical** |
| ameba-arduino-pro2 (releases) | V4.1.1-QC-V05 (Apr 30, 2026); V4.1.1 stable = HTTP 404; V4.1.2 = HTTP 404 | **No new release** |
| ameba-rtos-pro2 (main branch) | May 1, 2026 — SHA `1c1c8b7` (WLAN dhcp sync) | **No new commits — compare `1c1c8b7...main` = identical** |
| ameba-rtos-pro2 (tags) | V1.0.3-aiglass.07 (Apr 2, 2026) — most recent | **No new tags** |
| ameba-arduino-pro2 pull requests | 0 open; 319 closed; no FlashMemory/FCS/mutex PR ever filed | **No fix under review** |
| ameba-arduino-pro2 issues | 12 open; highest filed: #398 (Mar 29, 2026); #408–#410 = HTTP 404 | **Zero new FCS/FlashMemory/VOE issues** |
| ameba-rtos-pro2 issues | 3 open; highest: #16 (Jan 2026); #17 = HTTP 404 | **Zero new relevant issues** |
...
| Ai-Thinker-Open GitHub org | — | **No BW21-CBV repository** |
| ameba-arduino-pro2 forks (36 total) | — | **Zero forks contain FlashMemory mutex patch** |
| forum.amebaiot.com | Threads #4848–#4870 probed (all HTTP 403); highest indexed: #4847 (~May 6–7, 2026); no new FCS threads | **No new accessible bug-related content** |
| CSDN / Zhihu / 21ic / EEWorld | — | **Zero Chinese-language reports — reconfirmed** |
| bbs.ai-thinker.com (BW21-CBV) | Highest confirmed: tid=47223 (DIY camera, unrelated) | **No camera/FCS bug threads** |
| Amazon.com (BW21-CBV-Kit YELUFT, B0FY67YTFN) | Documented Finding 154, 2026-05-09 | **Retail unit carries unpatched SDK** |
...
- ameba-arduino-pro2 compare `13961cc...dev` (confirmed identical — no new commits since May 5, 2026): https://github.com/Ameba-AIoT/ameba-arduino-pro2/compare/13961cc...dev
- ameba-rtos-pro2 compare `1c1c8b7...main` (confirmed identical — no new commits since May 1, 2026): https://github.com/Ameba-AIoT/ameba-rtos-pro2/compare/1c1c8b7...main
- ameba-arduino-pro2 issues (12 open; highest #398 Mar 2026; #408–#410 = HTTP 404; zero new FCS issues): https://github.com/Ameba-AIoT/ameba-arduino-pro2/issues
- ameba-rtos-pro2 issues (3 open; highest #16 Jan 2026; #17 = HTTP 404): https://github.com/Ameba-AIoT/ameba-rtos-pro2/issues
- ideashatch/HUB-8735 issues (only issue #10 Aug 2025): https://github.com/ideashatch/HUB-8735/issues
- ameba-arduino-pro2 PRs (0 open; 319 closed; no FlashMemory/FCS PR): https://github.com/Ameba-AIoT/ameba-arduino-pro2/pulls
- ameba-arduino-pro2 releases (confirmed: V4.1.1-QC-V05 latest; V4.1.1 stable = HTTP 404; V4.1.2 = HTTP 404): https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases
- ameba-arduino-pro2 FlashMemory.cpp dev (re-confirmed no mutex, SHA `4fdfbec`, >9 months unmodified): https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp
- ameba-rtos-pro2 video_api.c main (re-confirmed unguarded ftl_common_write() calls; device_lock.h included but FCS path unguarded): https://raw.githubusercontent.com/Ameba-AIoT/ameba-rtos-pro2/main/component/video/driver/RTL8735B/video_api.c
- ameba-arduino-doc commits/main (re-confirmed last: `d0b6ca3`, Apr 16, 2026; no new commits; no FCS/FlashMemory warning): https://github.com/Ameba-AIoT/ameba-arduino-doc/commits/main
- forum.amebaiot.com threads #4848–#4870 (all HTTP 403; no FCS/flash camera content indexed): https://forum.amebaiot.com/t/4848 (representative URL)

---

## Research Update — 2026-05-10 (Update 2 — 6-hour cycle)

### Finding 162 — All Repositories Confirmed Static Since Previous Cycle; No Fix Found
**Source:** Direct compare fetches + commit-page fetches (2026-05-10, second 6-hour run)  
https://github.com/Ameba-AIoT/ameba-arduino-pro2/compare/13961cc...dev  
https://github.com/Ameba-AIoT/ameba-rtos-pro2/compare/1c1c8b7...main  
**Priority:** LOW — Status confirmation; no new fix commits
...
Direct compare endpoint `13961cc...dev` returns "identical". Direct compare endpoint `1c1c8b7...main` returns "identical". Zero new commits to either repository. No new release (V4.1.1-QC-V05 still the latest, confirmed via releases page). Zero open pull requests on `ameba-arduino-pro2`. `FlashMemory.cpp` remains unmodified since SHA `4fdfbec` (September 30, 2025) — now over 9 months without a mutex fix.

---

### Finding 163 — FlashMemory.cpp and video_api.c Re-Confirmed Unpatched (May 10, 2026 — Update 2)
**Source:** Direct raw fetches (2026-05-10, second run)  
https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp  
**Priority:** LOW — Direct re-verification; bug confirmed still present

Fresh fetch of `FlashMemory.cpp` (dev branch) confirms the `write()` function body is identical to SHA `4fdfbec`:
```cpp
void FlashMemoryClass::write(unsigned int offset)
{
    // ... bounds checks ...
...
**Zero calls** to `device_mutex_lock` or `device_mutex_unlock`. **Zero** `device_lock.h` includes. No guard of any kind around `flash_erase_sector()` or `flash_stream_write()`. The `writeWord()` function's fallback erase-rewrite path is equally unprotected. Unchanged from the September 30, 2025 "Optimize codes #337" commit.

---

### Finding 164 — Forum Threads #4848–#4855 Not Indexed; No New Bug-String Matches (May 10, 2026 Update 2)
**Source:** Google site-search + direct URL probes for forum.amebaiot.com threads above #4847 (2026-05-10, second run)  
**Priority:** LOW — Status confirmation

Direct URL probes of threads #4848–#4855 all return HTTP 403. A targeted Google site-search for `site:forum.amebaiot.com 4848 OR 4849 OR 4850 OR 4851 OR 4852 OR 4853 OR 4854 OR 4855` returns zero results — none of these threads are indexed. The highest confirmed indexed forum thread remains **#4847** ("I2C1 for MPU6050", ~May 6–7, 2026). No new forum posts in the indexed range describe any error matching `VOE_OPEN_CMD`, `FCS KM_status`, `slot full`, `sensor initial process`, or `FlashMemory`.

---

### Finding 165 — Zero New Chinese-Language and English Web Reports (May 10, 2026 Update 2)
**Source:** Web searches (Google indexed, 2026-05-10, second run) — English and Chinese  
**Priority:** LOW — Status confirmation
...
| `"FCS KM_status 0x00002081"` | **Zero results** |
| `"[VOE][WARN]slot full"` (Ameba context) | **Zero results** |
| `"VOE_OPEN_CMD fail" ameba RTL8735B` | **Zero results** |
| `"device_mutex_lock" "FlashMemory" Ameba` | **Zero results** |
| `AMB82 RTL8735B flash memory write camera boot fail VOE FCS May 2026 fix patch` | Zero relevant hits |
| `ameba-arduino-pro2 "device_mutex_lock" OR "RT_DEV_LOCK_FLASH" FlashMemory commit fix` | Zero results |
| `安信可 BW21-CBV AMB82 闪存写入 摄像头 FCS 冷启动失败 VOE 2026` (Chinese) | **Zero results** |
| `site:csdn.net AMB82 RTL8735B FlashMemory FCS camera cold boot 2026` | Zero CSDN results |
| `site:21ic.com OR site:eeworld.com.cn RTL8735B AMB82 flash camera FCS VOE 2026` | Zero Chinese EE-forum results |
| `site:forum.amebaiot.com camera flash FCS 2026` | Returns only thread #4302 (different bug, FCS mode *disabled*) |

All bug-signature strings continue to return zero indexed results. The root cause analysis (Hypothesis F — `FlashMemory.cpp` bypasses `RT_DEV_LOCK_FLASH`) remains uniquely documented in this research log.

---

### Complete Status Sweep — 2026-05-10 (Update 2)
...
| Ai-Thinker-Open GitHub org | — | **No BW21-CBV repository** |
| ameba-arduino-pro2 forks (36 total) | — | **Zero forks contain FlashMemory mutex patch** |
| forum.amebaiot.com | Threads #4848–#4855 probed (all HTTP 403 / unindexed); highest indexed: #4847 | **No new accessible bug-related content** |
| CSDN / Zhihu / 21ic / EEWorld | — | **Zero Chinese-language reports — reconfirmed** |
| bbs.ai-thinker.com (BW21-CBV) | — | **No camera/FCS bug threads** |
| FlashMemory.cpp (dev, SHA `4fdfbec`) | Sept 30, 2025 (>9 months unmodified) | **Still NO mutex fix — confirmed by direct raw fetch** |
| video_api.c (main) | March 3, 2026 | **Unguarded `ftl_common_write()` calls; no mutex fix** |
...
**No HIGH priority confirmed fix found. Bug status: publicly undocumented and unpatched as of 2026-05-10 (second 6-hour run).**

---

### Sources Added (Update 2026-05-10, Update 2)
...
- ameba-arduino-pro2 releases (re-confirmed: V4.1.1-QC-V05 latest; V4.1.1 stable = HTTP 404; V4.1.2 = HTTP 404): https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases
- ameba-arduino-pro2 FlashMemory.cpp dev (re-confirmed no mutex, `write()` body unchanged from SHA `4fdfbec`): https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp
- forum.amebaiot.com threads #4848–#4855 (all HTTP 403 / unindexed; no FCS/flash camera content): https://forum.amebaiot.com/t/4848 (representative URL)

---

## Research Update — 2026-05-11

**Cycle**: Recurring 6-hour automated research run — 2026-05-11
**Previous update**: 2026-05-10 (Update 2), ending at Finding 165
**Repositories checked**: ameba-arduino-pro2 (dev), ameba-rtos-pro2 (main), ideashatch/HUB-8735, Ai-Thinker-Open
**Search scope**: GitHub commits/PRs/issues/releases, forum.amebaiot.com, CSDN/Zhihu/21ic/EEWorld, bbs.ai-thinker.com, public bug-signature strings

---

### Finding 166 — ameba-arduino-pro2 dev branch: No new commits since May 5, 2026

**Priority**: LOW (status unchanged — no fix activity)
**Source**: https://github.com/Ameba-AIoT/ameba-arduino-pro2/commits/dev
**Finding**: The dev branch tip remains SHA `13961cc` (May 5, 2026, "Update API for AMB82-zero and SWD off logic"). Changes in that commit touched `wiring_digital.c`, `variant.cpp`, `variant.h` only — no FlashMemory, no FCS, no mutex. No new commits have landed since. The branch is static for the sixth consecutive 6-hour research cycle.

---

### Finding 167 — ameba-rtos-pro2 main branch: No new commits since May 1, 2026

**Priority**: LOW (status unchanged — no fix activity)
**Source**: https://github.com/Ameba-AIoT/ameba-rtos-pro2/commits/main
**Finding**: The main branch tip remains SHA `1c1c8b7` (May 1, 2026, "Sync upstream — wowlan dhcp renew"). No new commits have landed since. Both upstream repositories (arduino SDK + RTOS SDK) are simultaneously static, confirming no active development toward a fix.

---

### Finding 168 — V4.1.1-QC-V06 release: Explicitly confirmed HTTP 404

**Priority**: LOW (confirms no new release)
**Source**: https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases/tag/V4.1.1-QC-V06
**Finding**: Direct HTTP probe of V4.1.1-QC-V06 tag returns HTTP 404. V4.1.1 stable release also HTTP 404. V4.1.1-QC-V05 (April 30, 2026) remains the latest published release. Previously this absence was inferred from release-listing inspection; this cycle it is directly verified by probing the URL. No new release containing a FlashMemory mutex fix has shipped.

---

### Finding 169 — ameba-arduino-pro2 pull requests: 0 open, 319 closed — history confirmed clean

**Priority**: LOW (confirms no fix under review)
**Source**: https://github.com/Ameba-AIoT/ameba-arduino-pro2/pulls
**Finding**: ameba-arduino-pro2 has 0 open pull requests and 319 closed. Closed PR history was probed for any FlashMemory, FCS, mutex, device_lock, or VOE keywords — none found. No FlashMemory mutex patch has ever been submitted as a PR in this repository. The fix gap remains: no upstream contributor has proposed the two-line `device_mutex_lock` / `device_mutex_unlock` guard.

---

### Finding 170 — FlashMemory.cpp: SHA `4fdfbec` confirmed unpatched — 9+ months without modification

**Priority**: LOW (re-confirmation of root-cause gap)
**Source**: https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp
**Finding**: Direct raw fetch of `FlashMemory.cpp` confirms the file remains at SHA `4fdfbec` (September 30, 2025). Zero calls to `device_mutex_lock` or `device_mutex_unlock` anywhere in the file. Zero `#include "device_lock.h"` directives. The six unguarded raw SPI operations in `write()`, `writeWord()`, and `eraseSector()` are identical to the versions documented in previous research cycles. The file has not been modified in over 9 months.

Key unpatched `write()` body (confirmed identical to prior cycles):
```cpp
void FlashMemoryClass::write(unsigned int offset)
{
    if ((_flash_base_address + offset) < FLASH_MEMORY_APP_BASE) {
...
```
Zero mutex guards — root cause of the FCS flash race remains present.

---

### Finding 171 — video_api.c: Unguarded ftl_common_write() confirmed unpatched (March 3, 2026)

**Priority**: LOW (re-confirmation of complementary root-cause gap)
**Source**: https://github.com/Ameba-AIoT/ameba-rtos-pro2/blob/main/component/media/mmfv2/video_api.c
**Finding**: `video_api.c` last modified March 3, 2026. The `video_pre_init_save_cur_params()` function's SAVE_TO_FLASH path calls `ftl_common_write(0xF0D000, fcs_buf, 2048)` with no surrounding `device_mutex_lock(RT_DEV_LOCK_FLASH)` / `device_mutex_unlock(RT_DEV_LOCK_FLASH)` guards. The `#include "device_lock.h"` header is already present in the file — zero new includes would be needed for the fix. The two-line fix has not been applied.

---

### Finding 172 — All bug-signature strings: Zero new indexed results

**Priority**: LOW (confirms bug remains publicly undiscovered)
**Source**: Web search — multiple search engines
**Finding**: The following search strings each returned zero new indexed results in English or Chinese:
- `"It don't do the sensor initial process"` — zero results beyond prior-cycle snapshots
- `"FCS KM_status 0x00002081"` — zero results
- `"FCS KM_status" "0x0000200a"` — zero results
- `"[VOE][WARN]slot full"` — zero results
- `"VOE_OPEN_CMD fail" flash — zero results
- `"FlashMemory" "device_mutex_lock" Ameba` — zero results
- `"ftl_common_write" "device_mutex_lock"` — zero results

The root cause analysis (Hypothesis F — `FlashMemory.cpp` bypasses `RT_DEV_LOCK_FLASH`, causing concurrent SPI commands to abort the in-progress FCS page-program at `0xF0D000`) remains uniquely documented in this research log.

---

### Finding 173 — Chinese-language technical forums: Zero new reports

**Priority**: LOW (confirms bug is not circulating in Chinese embedded community)
**Source**: CSDN, Zhihu, 21ic, EEWorld, bbs.ai-thinker.com
**Finding**: No new posts on CSDN, Zhihu, 21ic.com, EEWorld, or bbs.ai-thinker.com reference any combination of: AMB82-mini / BW21-CBV / RTL8735B + camera boot failure + flash write + FCS error. The bug does not appear to have been independently reproduced or reported in the Chinese-language embedded engineering community as of this cycle.

---

### Finding 174 — forum.amebaiot.com: All probed threads (#4849–#4860) HTTP 403 or unindexed

**Priority**: LOW (no accessible new content)
**Source**: https://forum.amebaiot.com/t/4849 through /t/4860 (representative range)
**Finding**: forum.amebaiot.com continues to return HTTP 403 for all direct thread content. Google-indexed snippets for threads in the #4849–#4860 range show topics limited to USB connectivity, WLAN configuration, and general peripheral questions — no FCS camera boot failures, no FlashMemory-related issues. The forum gap analysis from prior cycles holds: no camera/FCS boot failure thread has been indexed since at least thread #4000.

---

### Finding 175 — ideashatch/HUB-8735 and Ai-Thinker-Open: No new activity

**Priority**: LOW (partner repos static)
**Source**: https://github.com/ideashatch/HUB-8735 | https://github.com/Ai-Thinker-Open
**Finding**: ideashatch/HUB-8735 remains at SHA `870a7e0` (December 2, 2025) with only issue #10 (August 2025). Ai-Thinker-Open GitHub organization has no BW21-CBV repository. Neither partner/reseller has published any FCS-related fix, workaround documentation, or errata for the RTL8735B platform.

---

### Complete Status Sweep — 2026-05-11

| Repository / Source | Last activity | Status |
|---|---|---|
| ameba-arduino-pro2 (dev branch) | May 5, 2026 — SHA `13961cc` (AMB82-zero SWD logic) | **No new commits — static 6th consecutive cycle** |
| ameba-arduino-pro2 (releases) | V4.1.1-QC-V05 (Apr 30, 2026); V4.1.1 stable = HTTP 404; V4.1.1-QC-V06 = HTTP 404 (directly verified) | **No new release** |
| ameba-rtos-pro2 (main branch) | May 1, 2026 — SHA `1c1c8b7` (WLAN dhcp sync) | **No new commits — static** |
| ameba-rtos-pro2 (tags) | V1.0.3-aiglass.07 (Apr 2, 2026) — most recent | **No new tags** |
| ameba-arduino-pro2 pull requests | 0 open; 319 closed; no FlashMemory/FCS/mutex PR ever filed | **No fix under review** |
| ameba-arduino-pro2 issues | 17 open; highest filed: #398 (Mar 29, 2026) | **Zero new FCS/FlashMemory/VOE issues** |
| ameba-rtos-pro2 issues | 3 open; highest: #16 (Jan 2026) | **Zero new relevant issues** |
| ideashatch/HUB-8735 | Dec 2, 2025 — SHA `870a7e0`; issue #10 only | **Inactive** |
| Ai-Thinker-Open GitHub org | — | **No BW21-CBV repository** |
| forum.amebaiot.com | Threads #4849–#4860 probed (all HTTP 403 / unindexed) | **No new accessible bug-related content** |
| CSDN / Zhihu / 21ic / EEWorld | — | **Zero Chinese-language reports — reconfirmed** |
| bbs.ai-thinker.com (BW21-CBV) | — | **No camera/FCS bug threads** |
| FlashMemory.cpp (dev, SHA `4fdfbec`) | Sept 30, 2025 (>9 months unmodified) | **Still NO mutex fix — confirmed by direct raw fetch** |
| video_api.c (main) | March 3, 2026 | **Unguarded `ftl_common_write()` calls; no mutex fix** |
...
| Public web (`"FCS KM_status 0x00002081"`) | — | **Zero new indexed results** |
| Public web (`"device_mutex_lock" "FlashMemory" Ameba`) | — | **Zero results — root cause uniquely documented in this log** |
| V4.1.1-QC-V06 release tag | — | **HTTP 404 — directly verified this cycle** |

**No HIGH priority confirmed fix found. Bug status: publicly undocumented and unpatched as of 2026-05-11.**

---

### Sources Added (Update 2026-05-11)
- ameba-arduino-pro2 dev commits (confirmed SHA `13961cc` still tip; no new commits since May 5, 2026): https://github.com/Ameba-AIoT/ameba-arduino-pro2/commits/dev
- ameba-rtos-pro2 main commits (confirmed SHA `1c1c8b7` still tip; no new commits since May 1, 2026): https://github.com/Ameba-AIoT/ameba-rtos-pro2/commits/main
- ameba-arduino-pro2 releases (V4.1.1-QC-V05 latest; V4.1.1-QC-V06 = HTTP 404 directly verified): https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases
- ameba-arduino-pro2 pulls (0 open, 319 closed, no FlashMemory/FCS/mutex PR ever filed): https://github.com/Ameba-AIoT/ameba-arduino-pro2/pulls
- FlashMemory.cpp dev raw (SHA `4fdfbec` confirmed; zero mutex guards; 9+ months unmodified): https://raw.githubusercontent.com/Ameba-AIoT/ameba-arduino-pro2/dev/Arduino_package/hardware/libraries/FlashMemory/src/FlashMemory.cpp
- video_api.c main (March 3, 2026; unguarded ftl_common_write in SAVE_TO_FLASH path): https://github.com/Ameba-AIoT/ameba-rtos-pro2/blob/main/component/media/mmfv2/video_api.c
- forum.amebaiot.com threads #4849–#4860 (all HTTP 403 / unindexed; no FCS/flash camera content): https://forum.amebaiot.com/t/4849 (representative URL)
- V4.1.1-QC-V06 release tag (HTTP 404 — directly verified): https://github.com/Ameba-AIoT/ameba-arduino-pro2/releases/tag/V4.1.1-QC-V06
